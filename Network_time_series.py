# -*- coding: utf-8 -*-
"""
Created on Mon Oct 12 11:58:18 2020
...
def createRandomSortedList(num,start = 0, end = 499):
    ...
    arr.sort()
    return arr
#x1=np.random.randint(500,size=(N1,1))
...
for k in range(0,5):
    ...
    i_1=[]
    for  i in range(0,N):
        i_1.append(np.max(sol1[:,5*i+2]))
  
    i_2=[]
    for i in range(0,tf*100):
        i_2.append(np.sum(sol1[i,2:5*N:5])/5000000.)
    i_peak.append(np.max(i_2))
        
    l=['SW','SI','SD','SB','SC']
    markers=["ko","b*","gP","ms","yX"]
    l2=['k','b','g','m','y']
    ind1=(0,50000,100000,150000)
    ind2=(0,.1,.2,.3,.4)
    pl.figure(1)
    ma_on=[0,10000,20000,30000,40000,50000,60000,70000,80000,90000,100000,110000,120000,130000,140000]
    pl.plot(i_2,l2[k])
    pl.plot(ma_on,i_2[0:150000:10000],markers[k],label=l[k])
    pl.xlabel('$t$',fontsize=30)
    pl.ylabel(r'$\mathcal{I}$',fontsize=30)
    pl.xticks(ind1,('0','500','1000','1500'),fontsize=25)
    pl.yticks(ind2,('0','0.1','0.2','0.3','0.4'),fontsize=25)
